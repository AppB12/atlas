...
def amazon_prod_info(keywords_list):
    global browser, prod_URL, prod_title, price, prods_info
    ...
    try:
        ...
        for i in range(0, len(keywords_list)):
            ...
            element.send_keys(keywords_list[i])  # Emulates entering keyword in search box
...
# To parse the API response only for review content
def clean_response(dict_data1):
    print "DICT_DATA1:"
    print type(dict_data1)
    print dict_data1
    print "REAL_DICT_DATA1:"
    dict_str = str(dict_data1)

    real_dict_data1 = dict_str[dict_str.find("group") + 9: dict_str.find("pageData") - 8]
    print "DICT_STR: " + real_dict_data1
    real_dict_data1 = real_dict_data1.replace('\n', " ")

    real_dict_data1 = ast.literal_eval(real_dict_data1)

    return real_dict_data1

# #######################################################################################


# Converting dictionary to data frame (to convert to CSV later)
def dict_to_data_frame(real_resp1, prod_URL1, rURL1):
    this_pg_reviews1 = pd.DataFrame({'rUser': [' '],
                                     'rUserLink': [' '],
                                     'rTitle': [' '],
                                     'rTitleLink': [' '],
                                     'rDate': [' '],
    ...
    one_review1 = pd.DataFrame({'rUser': [' '],
                                'rUserLink': [' '],
                                'rTitle': [' '],
                                'rTitleLink': [' '],
                                'rDate': [' '],
    ...
    print type(real_resp1)
    start_idx = [m.start() for m in re.finditer('rRating', str(real_resp1))]
    # rev_count = int(str(real_resp1).find("rDate"))
    rev_count = len(start_idx)
    print rev_count
    print "Count of reviews on this page: " + str(rev_count)
    review_content_heads = ['rUser', 'rTitle', 'rDate', 'rText', 'rRating']
    review_content = []
    for d in range(0, rev_count):  # loop through each review on this page
        review_content = []
        for rch in review_content_heads:
            dict1 = real_resp1[d][rch]
            # print dict1
            for d1 in dict1:
                print d1['text']
                review_content.append(d1['text'])
                if not rch == "rDate" and not rch == "rText":
                    print d1['href']
                    review_content.append(d1['href'])
                else:
                    pass
        print "@@@"
        if len(review_content) == 8:  # If all values are fetched
            one_review1 = pd.DataFrame({'rUser': [review_content[0]],
                                        'rUserLink': [review_content[1]],
                                        'rTitle': [review_content[2]],
                                        'rTitleLink': [review_content[3]],
                                        'rDate': [review_content[4]],
                                        'rText': [review_content[5]],
                                        'rRating': [review_content[6]],
                                        'rRatingLink': [review_content[7]],
                                        'rURL': [rURL1],
                                        'pURL': [prod_URL1]}, index=[0])
        else:  # If any error, and all values not fetched
            one_review1 = pd.DataFrame({'rUser': ['#N/A'],
                                        'rUserLink': ['#N/A'],
                                        'rTitle': ['#N/A'],
                                        'rTitleLink': ['#N/A'],
                                        'rDate': ['#N/A'],
                                        ...
                                        'rURL': [rURL1],
                                        'pURL': [prod_URL1]}, index=[0])
        this_pg_reviews1 = this_pg_reviews1.append(one_review1)
    return this_pg_reviews1

# #######################################################################################


# To extract reviews from API response
def get_reviews(resp, prod_URL, rev_URL):
...
def amazon_i_all_info(keywords_list):
    global browser, rev_url, prod_title, price, prods_info, all_reviews, all_kw_all_rev, review_urls_list
                ...
                while has_next_page:
                    ...
                    for u1 in range(0, 3):  # for u in prod_urls:  # change later
                        ...
                        for r2 in review_urls_list:
                            ...
                            # print type(response)
                            this_pg_reviews = get_reviews(response, prod_URL, r2)
