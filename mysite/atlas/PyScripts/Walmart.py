    ...
    try:
        ...
        if prod_rev_count > 0:
            while prod_rev_count1 > 0:
                # List of reviews on this response page
                ...
                # Negate number of reviews extracted from number of reviews remaining
    ...
    try:
        ...
        try:
            ...
            while has_next_page:
                ...
                prod_list = browser.find_elements_by_xpath("//div[@id='searchProductResult']//div[contains(@data-tl-id,'ProductTileListView-')]//div[@class='search-result-product-title listview']//a[1]")
                ...
                for t in temp_idx_list:
                    print("t=" + str(t))

                    # Product ID
                    print(all_prod_id_list[t])

                    # Product URL
                    final_prod_href_list.append(all_href[t])
                    print(all_href[t])

                    try:
                        browser1.get(all_href[t])
                        ...
                        # Product Price
                        try:
                            p_price = browser1.find_element_by_xpath("//div[contains(@class, 'prod-rightContainer')]//div[@class ='prod-PriceHero']").get_attribute("innerText")

                        except:
                            try:
                                p_price = browser1.find_element_by_xpath("//span[@data-tl-id='Price-ProductOffer']").get_attribute("innerText")
                            except:
                                p_price = "#N/A"
                    ...
                    prods_info = prods_info.append(pd.DataFrame({'pModel': [all_prod_id_list[t]],
                                                                 'pTitle': [p_title],
                                                                 'pBrand': [p_brand],
                                                                 'pURL': [all_href[t]],
                                                                 'price': [p_price]}, index=[0]))

                # Extract reviews of each product and append to aggregated data-frame
                for i in range(len(final_prod_id_list)):
                ...
                # Check if pagination present for Results page
                next_page_elem = browser.find_element_by_class_name("paginator-btn-next")
                try:  # If next page button exists, click
                    next_page_elem.click()
                    has_next_page = True
                    print("Going to next Results page...")
                except:  # Else break out of loop
                    has_next_page = False
